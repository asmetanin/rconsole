            ...
            function(vt100) {
                return function(e) {
                    ...
                    if (e.keyCode == 38 || e.keyCode == 40) {
                        ...
                        if (e.keyCode == 38 && vt100.consoleHistory.hasPrevious()) {
                            //up
                            line = vt100.consoleHistory.previous();
                        } else if (e.keyCode == 40 && vt100.consoleHistory.hasNext()) {
                            //down
                            line = vt100.consoleHistory.next();
                        }

                        if (line) {
                            // replace input line
                            
                            //vt100.putString(2, vt100.cursorY, line ? line : "");
                            vt100.renderString(line, true);
                        }
...
VT100.prototype.vt100 = function(s, resetLine) {
    resetLine = !resetLine ? false : true;
...
function ConsoleHistory() {
    this.linesHistory = [];
    this.currentLinePointer = -1;
}

ConsoleHistory.prototype.appendLine = function(line) {
    if (!line) {
        return false;
    }
    this.linesHistory.push(line);
    return true;
}

ConsoleHistory.prototype.hasPrevious = function() {
    if (!this.linesHistory.length) {
        // if history is empty
        return false;
    }

    if (this.currentLinePointer != -1 && !this.linesHistory[this.currentLinePointer - 1]) {
        return false;
    }
...
ConsoleHistory.prototype.hasNext = function() {
    if (!this.linesHistory.length) {
        // if history is empty
        return false;
    }

    if (this.currentLinePointer == -1) {
        // if it has not used history searching yet
        return false;
    } else if (!this.linesHistory[this.currentLinePointer + 1]) {
        return false;
    }
...
ConsoleHistory.prototype.previous = function() {
    var line;
    if (this.currentLinePointer == -1) {
        line = this.linesHistory[this.linesHistory.length - 1];
        this.currentLinePointer = this.linesHistory.length - 1;
    } else {
        line = this.linesHistory[this.currentLinePointer - 1];
        this.currentLinePointer--;
    }

    return line;
}

ConsoleHistory.prototype.next = function() {
    var line;
    if (this.currentLinePointer != -1) {
        line = this.linesHistory[this.currentLinePointer + 1];
        this.currentLinePointer++;
